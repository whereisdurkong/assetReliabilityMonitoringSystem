...
export default function AssetDashboard() {
    ...
    useEffect(() => {
        const incomingId = location.state?.asset_analysis_id;
        if (!incomingId || allReports.length === 0 || assets.length === 0) return;

        // Find the report
        const targetReport = allReports.find(r => r.asset_analysis_id === incomingId);
        if (!targetReport) return;

        // Find and set the matching asset
        const targetAsset = assets.find(a => a.asset_id === targetReport.asset_id);
        if (targetAsset) {
            setSelectedAsset(targetAsset);
        }

        // Set the report after a short delay to let historicalReports populate
        setTimeout(() => {
            setSelectedReport(targetReport);
        }, 100);

    }, [location.state, allReports, assets]);
                                                ...
                                                <div key={report.asset_analysis_id} style={{
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                                                        ...
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                            {/* Status moved to upper right */}
                                                            {/* <div style={{
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                                color: report.criticality_analysis_status === 'Good/Ok' ? '#4CAF50' :
                                                                    report.criticality_analysis_status === 'Verify/Abnormal' ? '#FF9800' :
                                                                        report.criticality_analysis_status === 'Severe' ? '#f44336' : 'rgba(255, 255, 255, 0.5)',
                                                                fontSize: '12px',
                                                                fontWeight: '600',
                                                                gap: '6px',
                                                            }}>
                                                                <FeatherIcon icon='alert-circle' size={14} color={'#ff8800'} />
                                                                {report.criticality_analysis_status || 'No status available'}
                                                            </div> */}

                                                            <div style={{
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                                color: calculateReportStatus(report) === 'Done' ? '#4CAF50' : '#FF9800',
                                                                fontSize: '12px',
                                                                fontWeight: '600',
                                                                gap: '6px',
                                                            }}>
