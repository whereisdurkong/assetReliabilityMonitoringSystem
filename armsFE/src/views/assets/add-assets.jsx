...
import {
    FiPlus, FiTrash2, FiSave, FiCalendar, FiMapPin, FiTag,
    FiBox, FiClipboard, FiCpu, FiGrid, FiArrowRight, FiArrowLeft,
    FiCheckCircle, FiAlertCircle, FiInfo, FiSliders, FiZap, FiDatabase, FiSettings, FiUsers
} from 'react-icons/fi';
import { motion, AnimatePresence } from 'framer-motion';
...
export default function AddAsset() {
        ...
        const fetchData = async () => {
            try {
                ...
                if (Array.isArray(res.data)) {
                    ...
                    // Create assigned location options from departments
                    const locationOptionsFromDepts = departments.map(dept => ({
                        value: dept,
                        label: dept.toLowerCase().replace(/\s+/g, '_').replace(/_/g, ' & '),
                    }));
    ...
    const Validation = () => {
        ...
        if (!formData.assignedLocation.trim()) {
            showAlertMessage('error', 'Empty Fields', 'Assigned Location is required');
            return false;
        }
        if (!formData.category.trim()) {
            showAlertMessage('error', 'Empty Fields', 'Asset Category is required');
    ...
    const getProgress = () => {
        ...
        let filled = 0;
        if (activeSection === 'basic') {
            const fields = ['assetName', 'location', 'assignedLocation', 'assetType', 'category'];
            total = fields.length;
            filled = fields.filter(f => formData[f].trim()).length;
        } else if (activeSection === 'details') {
            const fields = ['trivector', 'commissioningDate'];
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div>
                                    <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: '600', color: '#1E293B' }}>
                                        {activeSection === 'basic' && 'Asset Identification'}
                                        {activeSection === 'details' && 'Technical Classification'}
                                        {activeSection === 'components' && 'Component Configuration'}
                                    </h3>
                                    <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: '#64748B' }}>
                                        {activeSection === 'basic' && 'Enter the core identifying information for the asset'}
                                        {activeSection === 'details' && 'Define technical parameters and commissioning data'}
                                                    ...
                                                    <div style={{ marginBottom: '24px' }}>
                                                        <Form.Group>
                                                            <Form.Label style={{
                                                                ...
                                                                <FiMapPin size={14} />
                                                                Physical Location <span style={{ color: '#EF4444' }}>*</span>
                                                            </Form.Label>
                                                            <Form.Select
                                                                name="location"
                                                                value={formData.location}
                                                    ...
                                                    <div style={{ marginBottom: '24px' }}>
                                                        <Form.Group>
                                                            ...
                                                            <div style={{ fontSize: '0.7rem', color: '#64748B', marginTop: '4px' }}>
                                                                Select the department responsible for this asset
                                                            </div>
                                                        </Form.Group>
                                                    </div>
                                                </Col>
                                            ...
                                            <div style={{ marginBottom: '28px' }}>
                                                <div style={{
                                                    ...
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                        <FiDatabase size={20} color="#8B5CF6" />
                                                        <div>
                                                            <div style={{ fontWeight: '500', color: '#1E293B' }}>Component Configuration</div>
                                                            <div style={{ fontSize: '0.8rem', color: '#64748B' }}>Each asset requires at least one component</div>
                                                        </div>
                                                    </div>
                                                </div>
