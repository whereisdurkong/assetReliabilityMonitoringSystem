...
export default function SetupOptionView() {
    ...
    // Store original values to detect changes
    const [originalAssetLocations, setOriginalAssetLocations] = useState([{ location: '', subLocation: '' }]);
    const [originalAssetTypes, setOriginalAssetTypes] = useState(['']);
    const [originalAssetCategories, setOriginalAssetCategories] = useState(['']);
    const [originalComponentTypes, setOriginalComponentTypes] = useState(['']);
    ...
    const hasChanges = () => {
        const currentLocations = assetLocations
            .filter(l => l.location.trim() && l.subLocation.trim())
            .map(l => `${l.location.trim()} - ${l.subLocation.trim()}`);
        const originalLocations = originalAssetLocations
            .filter(l => l.location.trim() && l.subLocation.trim())
            .map(l => `${l.location.trim()} - ${l.subLocation.trim()}`);
    ...
    // Helper to parse stored "Mill - Fabrication" strings back into { location, subLocation }
    const parseLocationString = (str) => {
        const dashIndex = str.indexOf(' - ');
        if (dashIndex !== -1) {
            return {
                location: str.substring(0, dashIndex).trim(),
                subLocation: str.substring(dashIndex + 3).trim()
            };
        }
        return { location: str.trim(), subLocation: '' };
    };

    useEffect(() => {
        const fetchData = async () => {
            const defaultEmpty = [{ location: '', subLocation: '' }];
            ...
            if (!option_id) {
                ...
                setComponentTypes(['']);
                setOriginalAssetLocations(defaultEmpty);
                setOriginalAssetTypes(['']);
                setOriginalAssetCategories(['']);
                setOriginalComponentTypes(['']);
                return;
            }
            ...
            try {
                ...
                if (data && Object.keys(data).length > 0) {
                    let locations = defaultEmpty;
                    let types = [''];
                    let categories = [''];
                    let components = [''];

                    if (data.option_asset_location && data.option_asset_location.trim() !== '') {
                        locations = data.option_asset_location.split(',').map(item => parseLocationString(item));
                        setAssetLocations(locations);
                    } else {
                        setAssetLocations(defaultEmpty);
                    }
                    ...
                    setOriginalAssetLocations(locations.length > 0 ? locations : defaultEmpty);
                    setOriginalAssetTypes(types.length > 0 ? types : ['']);
                    setOriginalAssetCategories(categories.length > 0 ? categories : ['']);
                    setOriginalComponentTypes(components.length > 0 ? components : ['']);
                } else {
